...
async function main() {
  ...
  // Load current vault deployment
  const deploymentsDir = path.join(__dirname, "..", "deployments");
  const filePath = path.join(deploymentsDir, `${network.name}-vault.json`);

  if (!fs.existsSync(filePath)) {
    throw new Error(
      `No vault deployment found for network: ${network.name}. Deploy first.`,
    );
  }
  ...
  console.log("\nUpgrading PolareonVault implementation...");
  const upgraded = await upgrades.upgradeProxy(proxyAddress, PolareonVault, {
    kind: "uups",
  });
  ...
  console.log(
    `📄 Deployment info updated: deployments/${network.name}-vault.json`,
  );
}
